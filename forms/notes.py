...
class CreateNoteForm(FlaskForm):
    ...
    background_image = FileField(
        "Вы можете добавить красивую картинку в качестве баннера заметки"
    )
    background_color = ColorField(
        "Вы можете выбрать красивый цвет для своей заметки"
    )
...
class EditNoteForm(FlaskForm):
    ...
    text = TextAreaField("Заметка", validators=[DataRequired()])
    background_image = FileField(
        "Выберите картинку в качестве баннера заметки"
    )
    background_color = ColorField(
        "Измените цвет своей заметки"
    )

    submit = SubmitField("Изменить заметку")
